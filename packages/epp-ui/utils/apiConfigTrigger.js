...
class apiTrigger {
  ...
  */
  getDataSource({
    ...
    if (dataSourcePath === '' || dataSourcePath === undefined) {
      return dataObj;
    }
  ...
  fieldMapping({
    ...
    Object.keys(mappingList).forEach((keyName) => {
      ...
      const { newKeyName } = mappingList[keyName];
      newObj[newKeyName] = [];
      // 如果数据源中找不到配置表的字段，则跳过
      if (!oldKeyValue) return false;
      let newKeyValue = oldKeyValue;
      // 字段处理器
      const { fieldProcessor } = mappingList[keyName];
  ...
  sendRequest({ apiPath, reqData, resSuccessMsg, resErrorMsg, onResSuccess, onResError, onResFinally }) {
    const apiPromise = TRPC.newRequest(apiPath, reqData);
