...
`vueDataPath`:对应template模版上的层级

`apiDataPath`:api回调需要处理的参数

`responseConfig`:api响应配置

`fieldMapping`:字段映射，api返回字段替换本地配置数据

`transformResponse`:响应数据字段全局处理（data 参数是映射好的数据，需要返回一个对象

`onResSuccess`:接口成功后的回调

`onResError`:接口失败后的回调

`onResFinally`:接口无论成功或者失败都要进行的回调
